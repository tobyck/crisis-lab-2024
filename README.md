...
The `embedded/` directory contains all the code which runs embedded on a board:
...
5. Change the `serialport` to whichever port you are using to communicate with the Arduino, and change the WebSocket IP adress and port, in `client.js`

6. Run `./client.js` using Nodejs, this will not work without an internet connection. THIS ALSO WILL NOT RUN WITH BUN.
